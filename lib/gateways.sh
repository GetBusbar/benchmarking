...
#
# THE DEVIATION THIS EXISTS TO KILL. Any file that names a specific gateway or iterates a frozen list
# instead of calling in here can go stale the moment a gateway is added or removed: a hardcoded roster
# leaves a new gateway invisible, and a hardcoded default or container name only ever works for one
# gateway. Discovery is `gateways/*/gateway.sh` on disk, alphabetical (glob order), so a directory IS
# the registration and there is no list that can drift from the tree.
#
